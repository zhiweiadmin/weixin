	...
	MobileCalendar.prototype = {
		...
		bindEvent: function(type) {
			...
			function popupDate(e) {
				...
				//点击maskquxaio
				var mask_cancel = _self.gearDate.querySelector(".gearDate");
				mask_cancel.addEventListener('touchstart',closeMobileCalendar);
			...
			function popupTime(e) {
				_self.gearDate = document.createElement("div");
				_self.gearDate.className = "gearDate";
				_self.gearDate.innerHTML = '<div class="mask"></div><div class="date_ctrl slideInUp">' +
					'<div class="date_btn_box">' +
					'<div class="date_btn lcalendar_cancel">取消</div>' +
					'<div class="date_btn lcalendar_finish">确定</div>' +
					'</div>' +
				...
				document.body.appendChild(_self.gearDate);
				timeCtrlInit();
				var slideUpHeight=$(document.getElementsByClassName("slideInUp")).outerHeight(true);
				var screenHeight=document.documentElement.clientHeight;
				$(document.getElementsByClassName("mask")).css("height",screenHeight-slideUpHeight);
				var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
				lcalendar_finish.addEventListener('touchstart', finishMobileTime);
                //点击mask取消
				var mask = _self.gearDate.querySelector(".mask");
				mask.addEventListener("touchstart",closeMobileCalendar);
			...
			//	time_ss = time_ss > 9 ? time_ss : '0' + time_ss;
				...
				$(_self.trigger.childNodes[1].children[1].childNodes[0]).html((time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm);
				closeMobileCalendar(e);
			}
